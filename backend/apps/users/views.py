...
_HOD_ROLES          = {'hod'}
_MESS_ROLES         = {'mess'}
_SPACES_ROLES       = {'facility manager', 'receptionist', 'lab in-charge', 'librarian', 'principal'}
...
class CurrentUserView(APIView):
    ...
    def get(self, request):
        ...
        effective_role, has_active_override, override_expires_at = _get_effective_role(user)
...
class DashboardAPIView(APIView):
    ...
    def get(self, request):
        ...
        spaces = SpaceBooking.objects.filter(
            user=user, start_datetime__date__gte=today
        ...
        mess = MessBooking.objects.filter(
            user=user, booking_date__gte=today
        ).exclude(status='REJECTED').order_by('booking_date')
...
class RoleOverrideViewSet(ModelViewSet):
    queryset = RoleOverride.objects.all().select_related(
        'user', 'overridden_role', 'granted_by'
    ).order_by('-created_at')
