...
def get_raw_global_approvers(role_name):
    """
    Returns the full QuerySet of active users holding the given role,
    plus any users holding an active RoleOverride for that role.
...
def notify_approvers(
    ...
    """
    recipients = get_raw_global_approvers(role_name)
...
def notify_auto_approved_informational(
    ...
    auto-approved because the requester was the sole reachable
    approver at the time, and that they (the recipient) still hold
    standing approval authority over this resource.
    Must use is_actionable=False and email_override=False on every
    call to notify() -- this must never generate an email or an
    actionable item, regardless of category/domain email defaults.
    """
    ...
    title = f"{domain.capitalize()} Request Auto-Approved"
    message = (
        f"An auto-approved request was generated by {requester} for {resource}. "
        "No action is required from you, but you are receiving this notice because "
        "you hold approval authority for this resource."
    )

    for user in other_eligible_users:
        notify(
            user,
            Notification.Category.SYSTEM,
            title,
...
def get_raw_space_approvers(space, role_name):
    """
    Returns the full list of valid space approver CustomUser objects
    for a given space and role, evaluating most-specific-wins precedence.
    Does not exclude any users.
    """
...
def resolve_space_approver_recipients(booking, role_name):
    ...
    Calls get_raw_space_approvers() and then filters out the requester
    so that a requester is never notified of their own action.
    """
    raw_approvers = get_raw_space_approvers(booking.space, role_name)
    return [u for u in raw_approvers if u.id != booking.user_id]
