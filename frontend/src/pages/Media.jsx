...
function TeamFluidView({ teamData, dateStr }) {
  ...
  const dateLabel = isToday ? "Today" : new Date(dateStr).toLocaleDateString("en-IN", { month: "short", day: "numeric" })

  return (
    <div className="space-y-4 md:col-span-2">
      {/* Summary banner */}
      <div className={`flex flex-wrap items-center justify-between gap-4 rounded-xl border px-5 py-4 shadow-sm ${
        is_full
        ...
        <div>
          ...
          <p className={`mt-1 text-2xl font-bold ${
            is_full ? 'text-red-600' : free_crew <= 1 ? 'text-amber-600' : 'text-green-700'
          }`}>
            {free_crew} of {total_crew}
            <span className="ml-2 text-sm font-normal text-gray-500">
              {free_crew === 1 ? 'member available' : 'members available'} {isToday ? 'today' : `on ${dateLabel}`}
            </span>
          </p>
        </div>
        ...
        <span className={`rounded-full px-3 py-1 text-xs font-bold ${
          ...
          {is_full ? 'Fully Occupied' : free_crew <= 1 ? 'Limited Availability' : 'Available'}
        </span>
      </div>
      ...
      {total_crew > 0 && (
        <div className="rounded-xl border border-gray-100 bg-white px-5 py-4 shadow-sm">
          <div className="mb-2 flex items-center justify-between text-xs font-semibold">
            ...
            <span className={is_full ? 'text-red-600' : 'text-green-700'}>{pct}% available</span>
          </div>
          <div className="h-3 w-full overflow-hidden rounded-full bg-gray-100">
            <div
              className={`h-full rounded-full transition-all ${
                is_full ? 'bg-red-500' : free_crew <= 1 ? 'bg-amber-500' : 'bg-green-500'
              }`}
              style={{ width: `${pct}%` }}
            />
          </div>
      ...
      {is_full && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-5 py-4 shadow-sm">
          <p className="flex items-center gap-2 text-sm font-bold text-red-800">
            ...
            No crew available for new bookings {isToday ? 'today' : `on ${dateLabel}`}
          </p>
          <p className="mt-1 text-sm text-red-600">
            All {total_crew} media crew members are currently assigned to other bookings.
          </p>
        </div>
      )}
      ...
      <div>
        ...
        {booked_slots?.length === 0 ? (
          <div className="rounded-xl border border-gray-100 bg-white px-5 py-6 text-center shadow-sm">
            <p className="text-sm font-semibold text-gray-500">No bookings scheduled for {isToday ? 'today' : dateLabel}.</p>
          </div>
        ) : (
