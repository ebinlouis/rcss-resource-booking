...
const patchVehicle = async (id, payload) => {
...
function VehicleFormModal({ vehicle, onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await patchVehicle(vehicle.id, payload)
        toast.success("Vehicle updated successfully.")
      } else {
        await createVehicle(payload)
        toast.success("Vehicle added successfully.")
      }
      queryClient.invalidateQueries({ queryKey: ["fleet", "vehicles", "admin"] })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm">
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:bg-gray-100 hover:text-gray-600 rounded-full transition">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="flex items-center justify-between p-4 rounded-xl border border-gray-200 bg-gray-50/60">
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-700 focus:ring-offset-2 ${form.is_active ? "bg-green-600" : "bg-gray-300"}`}
            >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm">
      <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl text-center">
        ...
        <h2 className="text-xl font-bold text-gray-900 mb-1">Deactivate Vehicle?</h2>
        <p className="text-sm text-gray-500 mb-4">
          This vehicle has booking history so it can't be permanently deleted.
          Deactivating it will hide it from new bookings but preserve past records.
        </p>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={onConfirm} disabled={loading}
            className="flex-1 bg-orange-500 hover:bg-orange-600 text-white py-2.5 rounded-xl text-sm font-semibold transition disabled:opacity-70 flex justify-center items-center gap-2">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm">
      <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl text-center">
        ...
        <p className="text-sm text-gray-500 mb-4">This will permanently remove this vehicle. This cannot be undone.</p>
        <div className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 mb-5 text-left">
          <p className="text-sm font-semibold text-gray-900">{vehicle.name}</p>
          <p className="text-xs text-gray-500 mt-0.5">{vehicle.registration_number} · {vehicle.capacity} seats</p>
        </div>
...
function RowActions({ vehicle, onEdit, onDelete, onReactivate, reactivatingId }) {
...
export default function AdminVehiclesPage() {
  ...
  const handleDeleteAttempt = async () => {
    ...
    try {
      await api.delete(`/fleet/vehicles/${target.id}/`)
      queryClient.invalidateQueries({ queryKey: ["fleet", "vehicles", "admin"] })
      queryClient.invalidateQueries({ queryKey: ["fleet", "vehicles"] })
  ...
  const handleDeactivate = async () => {
    ...
    setActionLoading(true)
    try {
      await patchVehicle(deactivateTarget.id, { is_active: false })
      queryClient.invalidateQueries({ queryKey: ["fleet", "vehicles", "admin"] })
      queryClient.invalidateQueries({ queryKey: ["fleet", "vehicles"] })
      toast.success("Vehicle deactivated. It won't appear in new bookings.")
      setDeactivateTarget(null)
    } catch {
      toast.error("Could not deactivate vehicle.")
    } finally {
      setActionLoading(false)
    }
  }
  ...
  const handleReactivate = async (vehicle) => {
    ...
    try {
      await patchVehicle(vehicle.id, { is_active: true })
      queryClient.invalidateQueries({ queryKey: ["fleet", "vehicles", "admin"] })
      queryClient.invalidateQueries({ queryKey: ["fleet", "vehicles"] })
      toast.success(`${vehicle.name} reactivated successfully.`)
    } catch (err) {
      const msg = err?.response?.data?.detail || err?.response?.data?.error || "Could not reactivate vehicle."
      toast.error(msg)
    } finally {
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        {!isLoading && !isError && filtered.map((vehicle, idx) => (
          <div key={vehicle.id}>
            <div className={`hidden md:grid grid-cols-12 items-center px-6 py-4 transition ${
              ...
              <div className="col-span-3">
                <div className="flex items-center gap-3">
                  ...
                  <p className={`text-sm font-semibold ${vehicle.is_active ? "text-gray-900" : "text-gray-400"}`}>
                    {vehicle.name}
                  </p>
                </div>
              </div>
              ...
              <div className="col-span-3">
                <p className={`text-sm font-mono ${vehicle.is_active ? "text-gray-700" : "text-gray-400"}`}>
                  {vehicle.registration_number}
                </p>
              </div>

              <div className="col-span-2">
                <p className={`text-sm ${vehicle.is_active ? "text-gray-700" : "text-gray-400"}`}>
            ...
            {/* Mobile card */}
            <div className={`md:hidden mx-3 my-3 rounded-2xl border shadow-sm p-4 ${
              vehicle.is_active ? "bg-white border-gray-200" : "bg-gray-50 border-gray-100"
            }`}>
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className={`text-sm font-bold ${vehicle.is_active ? "text-gray-900" : "text-gray-400"}`}>{vehicle.name}</p>
