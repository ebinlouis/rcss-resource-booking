...
const ProfileCompletionCard = ({ user }) => {
  ...
  return (
    <div className={`bg-white rounded-xl border shadow-sm p-5 ${isComplete ? 'border-green-100' : 'border-amber-100'}`}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`text-xs font-bold px-2 py-0.5 rounded ${isComplete ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}>
      ...
      <div className="w-full h-1.5 bg-gray-100 rounded-full mb-4">
        <div
          className={`h-1.5 rounded-full transition-all ${isComplete ? 'bg-green-500' : 'bg-amber-400'}`}
          style={{ width: `${percent}%` }}
        />
      </div>
