...
function RoomCard({ room, onOpenAvailability }) {
  ...
  const formattedType = room.space_type
    ? room.space_type.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, l => l.toUpperCase())
    : "Space"
  ...
  const equipment = Array.isArray(room.built_in_equipment) ? room.built_in_equipment : []
  ...
  return (
    <div className="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300 overflow-hidden">
      ...
      {/* Top Section — image if available, pattern placeholder if not */}
      <div className="relative h-48 bg-gray-50 flex items-center justify-center overflow-hidden">
        ...
        {/* Overlay gradient so badges stay readable over images */}
        {image && (
            ...
            <div className="flex flex-wrap gap-1.5">
              {equipment.map((eq) => (
                <span key={eq.id}
                  ...
                  {eq.equipment_name}{eq.quantity > 1 ? ` ×${eq.quantity}` : ''}
                </span>
              ))}
            </div>
          </div>
